...
CREATE VIEW reported_comments AS
    SELECT 
        ...
        (SELECT username FROM users WHERE user.user_id = reports.target_user_id) AS target_username,
        reports.user_id,
        (SELECT username FROM users WHERE user.user_id = reports.user_id) AS complainee_username,
        reports.offense_id,
